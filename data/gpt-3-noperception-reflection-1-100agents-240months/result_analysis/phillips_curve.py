            ...
            if hasattr(env, 'world'):
                ...
                for attr in ['wage', 'wages', 'hourly_wage', 'wage_rate']:
                    if hasattr(world, attr):
                        ...
                        print(f"✓ 找到工资数据: {attr}, 长度={len(wage_history)}")
                        break
                
                if wage_history is not None:
                    return compute_inflation_from_history(wage_history, "工资")
                else:
                    print("  world 对象中没有找到工资相关属性")
                    print(f"  world 的属性: {[a for a in dir(world) if not a.startswith('_')][:20]}")
            else:
                print("  env 对象中没有 world 属性")
                print(f"  env 的属性: {[a for a in dir(env) if not a.startswith('_')][:20]}")
    ...
    for m in range(len(states)):
        ...
        for agent_id, agent_state in month_states.items():
            ...
            if isinstance(agent_state, dict):
                ...
                # 尝试1: state 中的 skill (技能水平)
                if 'state' in agent_state and isinstance(agent_state['state'], dict):
                    skill = agent_state['state'].get('skill', None)
                    if skill is not None:
                        wage = skill * 168  # 月工资 = 时薪 × 168小时
                
                # 尝试2: 直接的 wage 字段
                if wage is None:
                    wage = agent_state.get('wage', None)
                
                # 尝试3: endogenous 中的信息
                if wage is None and 'endogenous' in agent_state:
                    endo = agent_state['endogenous']
                    wage = endo.get('wage', None) or endo.get('monthly_wage', None)
        ...
        if m < 5:
            print(f"  月{m+1}: 平均工资={avg_wage:.2f}, 样本数={count}")
...
def main():
    ...
    if not wage_inflation_rates:
        ...
        print("\n或者检查 states 中是否包含 skill 或 wage 信息")
        return
